  ...
  Future<void> _handleLogin() async {
    ...
    try {
      ...
      if (adminDoc.exists) {
        ...
        // Check if admin is allocated
        final isAllocated = adminData['isAllocated'] ?? false;
        if (!isAllocated) {
          await _auth.signOut();
          Get.snackbar(
            "Access Denied",
            "Admin account is not allocated. Please contact administration.",
        ...
        // User is a verified admin, proceed to admin dashboard
        Get.offAll(() => const AdminDashboard());
      } else {
        // 3. Check if user exists in students collection
        final studentDoc =
            await _firestore.collection('students').doc(user.uid).get();

        if (studentDoc.exists) {
          // User is a student, show access denied
          await _auth.signOut();
          Get.snackbar(
            "Access Denied",
